...
#Visualization of the transformation funtions
ugrid = seq(-1,2,length.out = nrow(dat.s))
target_grid = tf$cast(matrix(ugrid, ncol=1), tf$float32)
hx = predict_h(thetaNN_l[[1]], parents_x, target_grid)
plot(ugrid, as.numeric(hx), type='l', xlim=c(0.9,1.1),ylim=c(5,30))
abline(v=c(0,1))
